...
double Regression::best_fit_slope(const Points& points){
	double numerator = 0;
...
double Regression::best_fit_y_int(const Points& points){
	std::list<double> x;
	std::list<double> y;
	for(int i = 0; i < points.get_size(); i++){
		x.push_back(points[i].x);
...
double Regression::guess_y(double x, const Points& points){
	double slope = best_fit_slope(points);
